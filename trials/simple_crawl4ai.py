...
SITE_URL_BUILDERS = {
    "amazon": lambda query: f"https://f128c5ec8a18.ngrok-free.app/search?q={urllib.parse.quote_plus(query)}",
    "flipkart": lambda query: f"https://f128c5ec8a18.ngrok-free.app/search?q={urllib.parse.quote_plus(query)}",
    "walmart": lambda query: f"https://f128c5ec8a18.ngrok-free.app/search?q={urllib.parse.quote_plus(query)}",
    "croma": lambda query: f"https://f128c5ec8a18.ngrok-free.app/search?q={urllib.parse.quote_plus(query)}",
    "tatacliq": lambda query: f"https://f128c5ec8a18.ngrok-free.app/search?q={urllib.parse.quote_plus(query)}",
    "duckduckgo": lambda query: f"https://f128c5ec8a18.ngrok-free.app/search?q={urllib.parse.quote_plus(query)}",
}
...
def build_source_url(site_key: str, query: str) -> str:
    ...
    builder = SITE_URL_BUILDERS.get(site_key)

    if not builder:
        print(f"Unkown site '{site_key}', using default site '{DEFAULT_SITE}'")
        builder = SITE_URL_BUILDERS[DEFAULT_SITE]
    ...
    try:
        async with AsyncWebCrawler(config=browser_conf) as crawler:
            for i, url in enumerate(paginated_urls, start=1):
                ...
                print("RAW MARKDOWN: ", result.markdown[:2500])
...
def main():
    ...
    while True:
        ...
        if "exit" in user_input:
        ...
        elif "2" in user_input or "two" in user_input or "prompt" in user_input or "scraping" in user_input:
            ...
            print("Would you like to save the results as CSV or Excel?")
